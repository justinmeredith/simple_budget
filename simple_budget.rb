...
def individual_savings_goal_calculations_func(members, number_of_people, savings_goal)
  if number_of_people == 1
    ...
    # These calculate various monthly amounts the user can save from their leftovers
    # automatically to present to the user as preset options for savings and time frames.
    preset_1 = [(members[user_member_answer][:leftovers] * 0.45), (savings_goal[:amount] / (members[user_member_answer][:leftovers] * 0.45))]
    preset_2 = [(members[user_member_answer][:leftovers] * 0.55), (savings_goal[:amount] / (members[user_member_answer][:leftovers] * 0.55))]
    preset_3 = [(members[user_member_answer][:leftovers] * 0.75), (savings_goal[:amount] / (members[user_member_answer][:leftovers] * 0.75))]
    ...
    if user_answer == "a"
      ...
      # this point.
      while true
        puts "\nYou have $#{sprintf('%.2f', members[user_member_answer][:leftovers])} left over each month, so don't try to save more than that each month."
        print "\nHow much would you like to save each month? $"
        monthly_savings = $stdin.gets.chomp.to_f

        if monthly_savings > members[user_member_answer][:leftovers]
          puts "\nYou shouldn't set a monthly saving goal that is greater than the amount you have"
          puts "leftover at the end of each month. You can always save more than you set for"
          puts "your monthly goal, but you should keep the goal realistic."
